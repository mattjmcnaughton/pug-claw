#!/usr/bin/env python3
"""Discord bot CLI for reading and writing server data via discord.py."""
    ...
    match command:
        ...
        case "send-message":
            ...
            async def run(client: discord.Client):
                ...
                output({"id": str(msg.id), "channelId": str(msg.channel.id)})
        ...
        case "create-channel":
            ...
            async def run(client: discord.Client):
                ...
                kwargs = {"name": name}
                if opts.get("topic"):
                    kwargs["topic"] = opts["topic"]
                if opts.get("category-id"):
                    category = await client.fetch_channel(int(opts["category-id"]))
                    kwargs["category"] = category
                channel = await guild.create_text_channel(**kwargs)
                output({"id": str(channel.id), "name": channel.name})
        ...
        case "modify-channel":
            ...
            async def run(client: discord.Client):
                ...
                output({"id": str(updated.id), "name": updated.name})
        ...
        case "delete-channel":
            ...
            async def run(client: discord.Client):
                ...
                output({"deleted": channel_id})

            await with_client(run)

        case "add-reaction":
            channel_id = opts.get("channel-id")
            message_id = opts.get("message-id")
            ...
            async def run(client: discord.Client):
                ...
                output({"reacted": True, "messageId": message_id, "emoji": emoji})
        ...
        case _:
            error(
                ...
                "send-message, create-channel, modify-channel, delete-channel, add-reaction"
            )
